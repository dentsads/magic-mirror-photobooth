...
export class Anim1Component implements OnInit {
  ...
  async handleEvent(eventId: string) {
    let duration: number = await this.getDuration(this.componentData.assetPath)
  ...
  redirectAfterMilliSeconds(milliseconds: number, eventId: string) {
    ...
    let routingService = this.routingService;
    let activatedRoute = this.activatedRoute;

    window.setTimeout(function() {
      let nextRoute = routingService.getNextRoute(activatedRoute, eventId)
  ...
  async getDuration(url): Promise<number> {
    ...
    var d = new Uint8Array(dataInBinary);
