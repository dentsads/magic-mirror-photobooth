...
inotifywait -m -e create --format '%f' "$PHOTOS_PATH/${EVENT_ID}" | while read file; do
    echo "{\"presigned_urls\":[]}" > $SCALED_PHOTOS_PATH/presigned-urls.json
    PRESIGNED_URL=$(aws s3 presign s3://magic-mirror-photobooth-gallery/$EVENT_ID/pics/$file --expires-in 604800)
    PRESIGNED_URL_JSON=$(jq '.presigned_urls += [{"file": "'$file'", "presigned_url": "'$PRESIGNED_URL'"}]' $SCALED_PHOTOS_PATH/presigned-urls.json)
    echo $PRESIGNED_URL_JSON > $SCALED_PHOTOS_PATH/presigned-urls.json
